...
export async function getUserGuilds(session) {
	...
	if (userGuilds.error || userGuilds.message == "401: Unauthorized") {
		return null;
		// 	throw new Error("Failed to fetch user guilds, try logging in again.");
	}
	...
	return adminUserGuilds.filter((guild) =>
		guildslist.some((botguild) => botguild.discordId === guild.id)
	);
}
